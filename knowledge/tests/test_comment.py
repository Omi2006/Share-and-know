...
class CommentTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        u1 = User.objects.create_user(username='Joe', password='Joe', email='Joe@joe.com')
    ...
    def test_get_post_comments(self):
        ...
        self.assertEqual(post_comments, [
            {'id': 1, 'commenter': {'username': 'Joe', 'email': 'Joe@joe.com'}, 'post': 1, 'content': 'Test1'}, 
            {'id': 2, 'commenter': {'username': 'Peter', 'email': 'Peter@peter.com'}, 'post': 1, 'content': 'Test2'}
        ])
    ...
    def test_create_new_post(self):
        ...
        self.assertDictEqual(loads(response.content), {'comment': 
            {'id': new_comment.id, 'commenter': {'username': 'Dan', 'email': 'Dan@dan.com'}, 'post': post.id, 'content': 'Test3'}
        })
        self.assertIn(new_comment, post.comments.all())
        self.assertIn(new_comment, User.objects.get(username='Dan').comments.all())
    ...
    def test_create_new_post_missing_fields(self):
        ...
        self.assertDictEqual(loads(response.content), {'errors': {'content': ['This field may not be blank.'], 'post': ['This field may not be null.']}})
    ...
    def test_edit_comment(self):
        ...
        self.assertDictEqual(loads(response.content), {'message': 'Comment edited successfully'})
    ...
    def test_edit_comment_other_user(self):
        ...
        self.assertDictEqual(loads(response.content), {'errors': {'comment': 'You can not edit this post!'}})
